...
public interface ClanInvitationRepository extends JpaRepository<ClanInvitation, Integer> {
    ...
    @Query("SELECT ci FROM ClanInvitation ci WHERE ci.expire_at > CURRENT_TIMESTAMP AND ci.player.id = :playerId AND ci.clan.id = :clanId AND ci.accepted_at IS NULL AND ci.declined_at IS NULL")
